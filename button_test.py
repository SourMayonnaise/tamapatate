...
GPIO.add_event_detect(26,GPIO.RISING,callback=button1_callback) # Setup event on pin 10 rising edge

GPIO.setup(6, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) # Set pin 12 to be an input pin and set initial value to be pulled low (off)
GPIO.add_event_detect(6,GPIO.RISING,callback=button2_callback) # Setup event on pin 12 rising edge

GPIO.setup(5, GPIO.IN, pull_up_down=GPIO.PUD_DOWN) # Set pin 16 to be an input pin and set initial value to be pulled low (off)
GPIO.add_event_detect(5,GPIO.RISING,callback=button3_callback) # Setup event on pin 16 rising edge
